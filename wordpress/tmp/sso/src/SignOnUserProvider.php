...
class SignOnUserProvider {
	...
	const WP_USER_CREATED_TIME = 'WP_USER_CREATED_TIME';
	const WP_LAST_LOGIN_TIME   = 'WP_LAST_LOGIN_TIME';
	const DEFAULT_USER_ROLE     = 'administrator';
	const WP_ADMIN_USER_EMAIL  = 'bitbucket@WP.com';
	...
	public function get_or_create_wp_user( $user_email, $first_name, $last_name, $role ) {
		...
		if ( $this->is_new_user( $user ) ) {
			if ( self::WP_ADMIN_USER_EMAIL === $user_email ) {
				throw new ImpersonatedUserException( 'The WP user was not found in the WordPress database during an attempted impersonation' );
			}
	...
	public function login_user( $user, $start_time, $request_id ) {
		...
		Logger::log( 'login_user', "email: { $email }, roles: { $roles }, response time(ms): { $elapsed_time }", $emai );
	}
	...
	protected function add_user_created_time_to_meta_data( $user_id ) {
		return add_user_meta( $user_id, self::WP_USER_CREATED_TIME, time() );
	}
	...
	private function update_last_login_time_to_meta_data( $user ) {
		return update_user_meta( $user->ID, self::WP_LAST_LOGIN_TIME, time() );
	}
	...
	private function create_new_wp_user( $user_email, $first_name, $last_name, $role ) {
		...
		if ( ! $this->add_user_created_time_to_meta_data( $new_user_id ) ) {
			$user_created = self::WP_USER_CREATED_TIME;
